...
def draw_boxes(img, bbox, identities=None):
    ...
    Y_ref = 290          #Head 150      #Sholder 165      #Center 225     #Foot = 290
    Y_up = Y_ref-60
    Y_down = Y_ref+60
    ...
    for i, box in enumerate(bbox):
        ...
        #x, y = (x1 + x2) // 2, (9*y1 + y2) // 10   #head
        #x, y = (x1 + x2) // 2, (8 * y1 + y2) // 8   #Sholder
        #x, y = (x1 + x2) // 2, (y1 + y2) // 2      #center
        x, y = (x1 + x2) // 2, (y1 + 50*y2) // 50   #foot
                ...
                if (y in range(Y_ref+1, Y_down)) :
                    if not label in down.keys():
                        ...
                        if label in up.keys():
                            ...
                            if label in time_in and label in time_out:   # Here t = Time_delay
                                t = abs(float(time_in[label][0] - float(time_out[label][0])))
                                if label in IN and label in OUT and t < 3.0 :
                                    IN.remove(label)
                                    OUT.remove(label)
...
def run_person_tracking(source=0, flip=False, use_popup=False, skip_first_frames=0):
    ...
    try:
        ...
        while True:
            ...
            #frame = frame[30:400, 250:500]    # ROI If you use ROI also change Time_filter ROI
            h, w = frame.shape[:2]
